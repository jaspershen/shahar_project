...
survey_data <-
  readr::read_csv("2_data/UPDATED FILE C1-6 With TimeStamps  4-24.csv")

survey_data2 <-
  readr::read_csv("2_data/66491TheLastingChang-Checkpoint5_DATA_2023-08-04_1646.csv")

###timestap for the samples
survey_data_old <- 
  survey_data
...
timestamp <-
  survey_data %>%
  dplyr::select(c(contains("_timestamp"))) %>% 
  dplyr::select(-c(contains("demographics_timestamp")))
...
sample_info <-
  ...
  purrr::map(function(x) {
    ...
    time1 <- c(
      x$study_surveys_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M"),
      x$study_surveys_2_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M")
    )
    time2 <- c(
      x$study_surveys1_checkpoint2_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M"),
      x$study_surveys2_checkpoint2_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M")
    )
    time3 <- c(
      x$study_surveys1_checkpoint3_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M"),
      x$study_surveys2_checkpoint3_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M")
    )
    time4 <- c(
      x$study_surveys1_checkpoint4_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M"),
      x$study_surveys2_checkpoint4_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M")
    )
    
    time5 <- c(
      x$study_surveys1_checkpoint5_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M"),
      x$study_surveys2_checkpoint5_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M")
    )
    
    time6 <- c(
      x$study_surveys1_checkpoint6_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M"),
      x$study_surveys2_checkpoint6_timestamp %>% as.POSIXct(tz = "UTC", tryFormats = "%m/%d/%y %H:%M")
    )
